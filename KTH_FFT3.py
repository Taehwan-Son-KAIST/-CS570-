...
import torch
from models.model_FFT3 import hardwire_layer
...
def make_raw_dataset(directory="kth-data-fft3-0.5", transform=None, f=9, device=None):
    """
    ...
    for subject_id in range(subjects):
        ...
        # hardwiring layer
        input = hardwire_layer(input, device, verbose=True).cpu() # Tensor shape : (N, f, h, w) -> (N, 1, 2f, h, w)
        ...
        data = {
            "category": categories,
            "input": input, # Tensor shape : (N, 1, 2f, h, w)
            "subject": subject_id
        }
